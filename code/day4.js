const data =`bdwdjjo avricm cjbmj ran lmfsom ivsof
...
class Day4 {
    ...
    createArrayFromString (data) {
        ...
        resultingArray.map((arr, key) => resultingArray[key] = arr.split(' '));
    ...
    checkForDuplicateWords (array) {
        ...
        while(noresult) {
            ...
            array.forEach((word, i) => {
                if (count !== i && (word === array[count] || this.isAnagram(word, array[count] )) ) {
                    // found a match
                    res = false;
                    noresult = false;
                }
    ...
    getResult (data) {
        let count = 0;
        this.createArrayFromString(data).map(arr => this.checkForDuplicateWords(arr) == true ? count++ : '' );
        console.log(count);
        return this.createArrayFromString(data).map(arr => this.checkForDuplicateWords(arr));
    }
}

new Day4().getResult(data);